...
function pricer(mcProcess::FinancialMonteCarlo.BaseProcess, zero_rate::FinancialMonteCarlo.AbstractZeroRateCurve, method::FinancialFFT.LewisMethod, abstractPayoff::FinancialMonteCarlo.BinaryEuropeanOption, ::FinancialMonteCarlo.BaseMode = FinancialMonteCarlo.SerialMode())
    T = abstractPayoff.T
    K = abstractPayoff.K
    A = method.A
    N = method.N
    ...
    CharFunc(v) = cf(v) * exp(-v * 1im * corr)
    mod = log(S0 / K) + (r - d) * T
    func_(z) = FinancialFFT.real_mod(exp(-z * 1im * mod) * CharFunc(-z - 1im * 0.5) / (0.5-z * 1im))
    int_1 = FinancialFFT.midpoint_definite_integral(func_, -A, A, N)
	price = (S0/K)* exp(-d * T)*exp(-mod / 2) * int_1 / (2 * pi)
    return FinancialFFT.call_to_put(price, mcProcess.underlying, zero_rate, abstractPayoff)
end
