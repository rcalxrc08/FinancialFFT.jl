...
const method_l2 = LewisMethod(A, N_lewis);
S0 = 100.0;
...
function blsprice_lewis(S0, K, r, T, sigma, d, A)
    price = pricer(BlackScholesProcess(sigma, Underlying(S0, d)), ZeroRate(r), LewisMethod(A, N_lewis), EuropeanOption(T, K))
    return price
end

@show Zygote.gradient(blsprice, S0, K, r, T, sigma, d)
@show Zygote.gradient(blsprice_lewis, S0, K, r, T, sigma, d, A)

function blsbin_lewis(S0, K, r, T, sigma, d, A)
    price = pricer(BlackScholesProcess(sigma, Underlying(S0, d)), ZeroRate(r), LewisMethod(A, N_lewis), BinaryEuropeanOption(T, K))
    return price
end
...
res_analytic_eu_lewis = Zygote.gradient(blsprice_lewis, S0, K, r, T, sigma, d, A)
...
res_analytic_eu_lewis = blsprice_lewis(S0, K, r, T, sigma_d, d, A)
...
res_analytic_eu_lewis = blsprice_lewis(S0, K, r, T, sigma_h, d, A)
@test abs(res_analytic_eu.value - res_analytic_eu_lewis.value) < toll
