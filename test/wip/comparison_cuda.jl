using FinancialToolbox, Test, DualNumbers, CUDA, FinancialFFT, FinancialMonteCarlo
...
result_cm = pricer(Model, zero_rate, method_cm, EUData, FinancialMonteCarlo.CudaMode());
method_lewis = LewisMethod(700.0, 200000);
result_lewis = pricer(Model, zero_rate, method_lewis, EUData, FinancialMonteCarlo.CudaMode());
method_cm_lewis = CarrMadanLewisMethod(A, N);
result_cm_lewis = pricer(Model, zero_rate, method_cm_lewis, EUData, FinancialMonteCarlo.CudaMode());
toll = 1e-2;
